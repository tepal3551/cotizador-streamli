...
# ==============================================================================
# SECCIÓN 1: CARGA DE CATÁLOGOS BLINDEADA Y DE ALTA VELOCIDAD
# ==============================================================================

@st.cache_data(show_spinner=False)
def cargar_catalogo(nombre_archivo_catalogo, nombre_archivo_actualizaciones):
    """Carga el catálogo en memoria de forma permanente con máxima velocidad."""
    try:
        # Uso de motor 'c' y tipos de datos optimizados para no saturar la RAM gratuita
        df = pd.read_csv(
...
def cargar_clientes_desde_sheets():
    """Descarga clientes desde Google Sheets con protección anti-congelamiento (Timeout 3s)."""
    ...
    try:
        respuesta = requests.get(url_clientes, timeout=3)
    ...
    # Respaldo inmediato si la nube tarda para no paralizar al vendedor
    return pd.DataFrame([
...
def analizar_y_cargar_pedido(texto_pedido, df_catalogo):
    """Analiza líneas de texto identificando código y cantidad rápidamente."""
